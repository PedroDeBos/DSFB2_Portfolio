...
#########################################################################################
# HAVE TO DO THIS PART AGAIN, USE STR COMMANDS INSTEAD OF USING ==, == IS DEPENDANT ON THE ORDER OF THE COUNTRIES
...
dengue_flu_countries<-flu$country %>% str_extract(pattern = dengue_countries) %>% unique() x
...
dengue_year<-dengue$year %>% unique()
flu_year<-flu$year %>% unique()
dengue_year == flu_year
# Dengue_year and flu_year are the exact same, thus we can just take those years: 2002:2015

gap_filter1<-gapminder %>% filter(year <= max(flu_year) & year >=min(flu_year)) #Filtering for the years

test<-data.frame()
for (x in dengue_flu_countries){
    data<-filter(gap_filter1, country == x)
    print(data)
  }
filter(paste("country == ", dengue_flu_countries))
...
And finally, a correlation between dengue incidence and flu incidence? (Answer: no. Not at all.)

```{r}
data_brazil<-data_full %>% filter(country=="Brazil") %>% group_by(year) %>% summarise(
  mean=mean(gdp),
  sd=sd(infant_mortality)
)

data_brazil %>% ggplot(aes(x=as.factor(year), y=mean))+
  geom_col(color="black")+
  theme_bw()

data_brazil<-data_full %>% filter(country=="Brazil") %>% group_by(year) %>% summarise(
  infant_mortality=mean(infant_mortality),
  flu_incidence=mean(flu_incidence)
)

data_brazil %>% ggplot(aes(x=infant_mortality, y=flu_incidence))+
  geom_point()+
  geom_line()
