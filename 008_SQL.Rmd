...
Now the data is tidy, however, to make the dengue/flu data relational, the "date" data needs to be properly synchronized. To do this, the "date" will be split into "year", "month" and "day".

```{r}
#Think separating would be better instead of just removing that information, never suffer information loss!!!
...
Using the SQL script shown in the screenshot below, a SQL database has been created called "workflowsdb". Now, using RPostgreSQL, the created datasets will be inserted into this database using the command "dbWriteTable
...
To inspect the database, another SQL script has been used. Once again, a screenshot of this script has been added underneath
