...
- Year: `r parameter$year` (Shows data from `r parameter$year`)

- Month: `r parameter$month` (Shows data from months number `r parameter$month`)
...
covid_data_vis_filter1 %>% group_by(month, year) %>% dplyr::summarise(
  ...
  theme(axis.text.x = element_text(angle=45, hjust=1))+
  labs(
    title=paste0("SARS-COV 2 deaths in ",parameter$country, " per month in ", parameter$year),
    subtitle = "Data from ECDC",
    x="Month",
    y="COVID-19 deaths",
    color="Year"
