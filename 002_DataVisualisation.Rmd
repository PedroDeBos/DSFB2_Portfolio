...
```{r InstallingAllPackages}
packages<-c("rmarkdown", "bookdown", "tidyverse", "drc", "readxl", "here", "ggplot2", "DT", "fs", "devtools", "dslabs", "RPostgreSQL", "RPostgres", "DBI", "multcomp", "plyr", "gmodels", "mlbench", "caret", "BiocManager", "randomForest", "gridExtra", "gmodels")
...
if(!require("PokemonAnalyse", character.only = TRUE)){
  install_github("PedroDeBos/PokemonAnalyse", dependencies = TRUE)
...
Celegans_data_normalised_sum<-Celegans_data_select_normalised %>% dplyr::filter(!is.na(normalised_RawData_percentage)) %>% dplyr::group_by(compConcentration, compName) %>% dplyr::summarise(
