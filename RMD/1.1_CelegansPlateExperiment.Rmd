...
Celegans_data_noNA<-Celegans_data %>% na.omit() #Removing NA's from the dataset so a summarise() works properly
Celegans_data_noNA %>% group_by(compName) %>% summarise(
  mean=mean(RawData),
  normalise_factor=mean/85.9
)
````
